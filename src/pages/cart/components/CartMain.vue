...
const isSelectedAll = computed(() => {
  //every对于空数组来说会返回true
  //判断购物车列表是否满足两个条件：列表非空 且 列表中所有商品都被选中
  //数组的 every() 是 ES6 数组方法，用于检测数组中所有元素是否都满足指定条件；
...
const selectedCartListCount = computed(() => {
  //，专门用来遍历数组并将所有元素 “归并” 成一个最终值（比如求和、求积、拼接字符串、转换对象等）
  return selectedCartList.value.reduce((sum, item) => sum + item.count, 0)
})
//计算选中总金额
const selectedCartListMoney = computed(() => {
  return selectedCartList.value
    .reduce((sumPrice, item) => sumPrice + item.count * item.nowPrice, 0)
    .toFixed(2)
})
// 结算按钮
const gotoPayment = () => {
